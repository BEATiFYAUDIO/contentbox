...
export default function RightsHoldersPage({ onOpenLegacyCatalog }: { onOpenLegacyCatalog?: () => void }) {
  ...
  async function discover(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const result = await api<LookupResponse>("/api/connect-work/lookup", "POST", { type, value: nextValue });
      setLookup(result);
      if (!result.discovery?.title) {
        setMessage("Identifier validated. No rich metadata was found, but you can still connect the work.");
      }
    } catch (err: any) {
  ...
  async function connectWork() {
    ...
    try {
      ...
      const result = await api<ConnectResponse>("/api/connect-work/connect", "POST", {
        ...
        value: lookup.identifier.value,
        title: discovery.title || `${lookup.identifier.type} ${lookup.identifier.displayValue}`,
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-neutral-800 bg-neutral-900/20 p-6">
        ...
        <div className="mt-1 text-sm text-neutral-400">
          Enter an industry identifier, review discovered metadata, then create a private Legacy asset in your catalog.
        </div>

        <form onSubmit={discover} className="mt-4 grid gap-3 md:grid-cols-[180px_1fr_auto]">
          <div>
          ...
          <div>
            <label className="mb-1 block text-xs text-neutral-400" htmlFor="connect-work-value">Identifier value</label>
            <input
