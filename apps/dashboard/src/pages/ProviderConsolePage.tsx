...
export default function ProviderConsolePage({ onOpenLightningConfig }: { onOpenLightningConfig?: () => void }) {
  ...
  const creatorScopedContentRows = useMemo(() => {
    ...
    const scopedIntentKeySet = new Set<string>();
    creatorScopedPaymentIntents.forEach((row) => {
      if (row.id) scopedIntentKeySet.add(row.id);
      if (row.paymentIntentId) scopedIntentKeySet.add(row.paymentIntentId);
    });
    creatorAllPaymentIntents.forEach((row) => {
      ...
      const inScope = (row.id && scopedIntentKeySet.has(row.id)) || (row.paymentIntentId && scopedIntentKeySet.has(row.paymentIntentId));
    ...
    <div className="space-y-5">
      ...
      {creatorScopeId !== "all" ? (
        <div className="rounded-xl border border-neutral-800 bg-neutral-900/30 p-4">
          ...
          <div className="mt-1 text-xs text-neutral-500">
            Paid/Payable are for the selected scope window only.
          </div>
          <div className="mt-2">
            <button
              type="button"
