...
async function main() {
  ...
  console.log("Macaroon length:", String(macaroon.length));
  ...
  // getinfo (optional)
  try {
    const infoRes = await fetchWithTimeout(`${baseUrl}/v1/getinfo`, { method: "GET", headers, dispatcher } as any, 8000);
    if (infoRes.ok) {
      const info = await infoRes.json();
      console.log("getinfo OK:", {
        alias: info?.alias || null,
        pubkey: info?.identity_pubkey || null,
        network: info?.chains?.[0]?.network || null
      });
    } else {
      console.warn("getinfo skipped:", await infoRes.text());
    }
  } catch (e: any) {
    console.warn("getinfo skipped:", String(e?.message || e));
  }

  // create invoice
  const memo = `ContentBox doctor ${new Date().toISOString()}`;
  ...
  console.log("invoice OK:", { bolt11Length: String(bolt11.length), providerIdHex: providerIdHex.slice(0, 8) + "…" });
...
main().catch((err) => {
  console.error("Doctor failed:", String(err?.message || err));
  process.exit(99);
});
